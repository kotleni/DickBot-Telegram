...
class DicksGame implements Api {
  ...
  async start() {
    ...
    this.bot.onText(/\//, async msg => {
      const isBanned = this.config.isBanned(msg.from?.id.toString() ?? '');
      const isGroupRegistered = this.config.isGroupAllowed(msg?.chat?.id);

      if (!isGroupRegistered) {
        await this.replyTo(msg, 'Цей бот не прикріпленний до цієї группи.');
        return;
      }
      ...
      await this.commandsManager.processCommandMessage(msg, this.config, this);
    });
